...
local old_Nerium = CoreGui:FindFirstChild('Nerium')

if old_Nerium then
    Debris:AddItem(old_Nerium, 0)
end

if not isfolder("Nerium") then
    makefolder("Nerium")
end
...
local Config = setmetatable({
    save = function(self: any, file_name: any, config: any)
        local success_save, result = pcall(function()
            ...
            writefile('Nerium/'..file_name..'.json', flags)
        end)
    ...
    load = function(self: any, file_name: any, config: any)
        local success_load, result = pcall(function()
            if not isfile('Nerium/'..file_name..'.json') then
                self:save(file_name, config)
        
                return
            end
        
            local flags = readfile('Nerium/'..file_name..'.json')
...
function Library:create_ui()
    ...
    local Nerium = Instance.new('ScreenGui')
    Nerium.ResetOnSpawn = false
    Nerium.Name = 'Nerium'
    Nerium.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    Nerium.Parent = CoreGui
    ...
    Icon.Image = 'rbxassetid://102347466457412' -- frame inicial
    ...
    -- IDs dos frames do GIF (novos ids fornecidos pelo usuário)
    local gifFrames = {
    ...
    function self:UIVisiblity()
        Nerium.Enabled = not Nerium.Enabled;
    end;
    ...
    function self:load()
        ...
        for _, object in Nerium:GetDescendants() do
    ...
    function self:create_tab(title: string, icon: string)
        ...
        function TabManager:create_module(settings: any)
            ...
            if not settings.rich then
                ...
                ModuleName.Text = settings.richtext or "<font color='rgb(255,0,0)'>Nerium</font> user"
            end;
            ...
            function ModuleManager:create_paragraph(settings: any)
                ...
                if not settings.rich then
                    ...
                    Body.RichText = true
                    Body.Text = settings.richtext or "<font color='rgb(255,0,0)'>Nerium</font> user"
                end
            ...
            function ModuleManager:create_text(settings: any)
                ...
                if not settings.rich then
                    ...
                    Body.RichText = true
                    Body.Text = settings.richtext or "<font color='rgb(255,0,0)'>Nerium</font> user" -- Default rich text
                end
                ...
                function TextManager:Set(new_settings)
                    if not new_settings.rich then
                        ...
                        Body.RichText = true
                        Body.Text = new_settings.richtext or "<font color='rgb(255,0,0)'>Nerium</font> user" -- Default rich text
                    end
                end;
            ...
            function ModuleManager:create_feature(settings)
                ...
                if not settings.disablecheck then
                    ...
                    Checkbox.BackgroundColor3 = checked and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(32, 38, 51)
                    Checkbox.Text = ""
...
local main1 = main:create_tab('Blatant', 'rbxassetid://76499042599127')
